...
def teaid_genome_blast(input_file, genome_file, output_dir, e_value, num_threads=1):
    ...
    # Check if provided genome file exist
        ...
        click.echo("WARNING: You provided genome file not found. TEAid can't perform genome blast.")
        return False
    ...
    if blast_database == "makeblastdb_not_found":
        click.echo("Error: makeblastdb command not found. TEAid can't perform genome blast.")
        return False

    # Check if error happened
    elif blast_database == "makeblastdb_got_error":
        click.echo("Error: BLAST database can't be established for the genome. TEAid can't perform genome blast.")
        return False
    ...
    if genome_blast_out == "blastn_not_found":
        ...
        click.echo("TEAid can't perform genome blast. Refer to terminal for more information.")
        return False
...
def self_blast(input_file, output_dir):
    ...
    # Check if makeblastdb is correctly installed
    if blast_database == "makeblastdb_not_found":
        click.echo("Error: makeblastdb command not found. TEAid can't perform self blast.")
        return False

    # Check if error happened
    elif blast_database == "makeblastdb_got_error":
        click.echo("Error: BLAST database can't be established for the genome. TEAid can't perform self blast.")
        return False

    self_blast_out, _ = blast(input_file, blast_database, output_dir_self_blast, e_value=0.05,
    ...
    if self_blast_out == "blastn_not_found":
        ...
        click.echo("TEAid can't perform self blast. Refer to terminal for more information.")
        return False
...
def plot_rpsblast_hits(input_file, cons_len):
    ...
    df_rps.columns = [
        ...
        'gapopen', 'qstart', 'qend', 'sstart', 'send', 'evalue', 'score', 'description'
    ]
    ...
    def assign_tracks(df_rps_assign):
        ...
        df_rps_assign_sorted = df_rps_assign.sort_values(by=['start', 'end']).reset_index(drop=True)
        ...
        for idx, hit in df_rps_assign_sorted.iterrows():
            ...
            for track_num, track_end in enumerate(tracks):
                if hit['start'] > track_end + (cons_len / 20):  # Adjusted gap to prevent overlaps
    ...
    #########################################################
    score_ranges = [
        {'min_score': float('-inf'), 'max_score': -1000, 'color': 'white', 'label': 'Alignment Score'},  # Show text only
    ...
    for idx, hit in df_rps_with_tracks.iterrows():
        ...
        if len(description_parts) > 1:
            brief_info = description_parts[1].strip()  # Take the second part of the description
        else:
        ...
        if direction == 'forward':
            ...
            x_coords = [
                plot_x_start, plot_x_end - arrow_head_length, plot_x_end - arrow_head_length, plot_x_end,
                plot_x_end - arrow_head_length, plot_x_end - arrow_head_length, plot_x_start, plot_x_start
            ]
            y_coords = [
                y + arrow_width, y + arrow_width, y + arrow_width * 1.5, y,
                y - arrow_width * 1.5, y - arrow_width, y - arrow_width, y + arrow_width
            ]
        else:
            # Coordinates for reverse arrow
            x_coords = [
                plot_x_end, plot_x_start + arrow_head_length, plot_x_start + arrow_head_length, plot_x_start,
                plot_x_start + arrow_head_length, plot_x_start + arrow_head_length, plot_x_end, plot_x_end
            ]
            y_coords = [
                y + arrow_width, y + arrow_width, y + arrow_width * 1.5, y,
                y - arrow_width * 1.5, y - arrow_width, y - arrow_width, y + arrow_width
...
def teaid_plotter(input_file, output_dir, genome_file, current_win, prepared_cdd = None, e_value=1e-40,
    ...
    if genome_blast_out:
        ...
        if genome_blast_out == "blast_n_zero":
            ...
            fig_coverage = empty_plot(cons_len, custom_text="No genome BLAST hits found")
            coverage_max = 10
        else:
        ...
        fig_blast = empty_plot(cons_len, custom_text= "An error occurred. Please check the terminal for details.<br> "
                                                      "Please provide a genome file if it has not been specified.")
        divergence_max = 10
        blast_plot_title = "BLAST hit Plot"

        fig_coverage = empty_plot(cons_len, custom_text="An error occurred. Please check the terminal for details.<br> "
                                                        "Please provide a genome file if it has not been specified.")
        coverage_max = 10
    ...
    if dotplot_blast:
        ...
        fig_dot = empty_plot(cons_len, custom_text= "An error occurred. Please check the terminal for details.")

    # Perform rpstblastn search
    if prepared_cdd is not None:
        rpstblastn_out = rpstblastn(con_seq, prepared_cdd, output_dir, e_value=0.01, os_type=os_type, num_threads=5)

        if rpstblastn_out:
            if rpstblastn_out == "rpstblastn_n_zero":
                fig_rpstblastn = empty_plot(cons_len, width_n=1350, height_n=60, custom_text="No protein domains found")
                num_tracks = 1
