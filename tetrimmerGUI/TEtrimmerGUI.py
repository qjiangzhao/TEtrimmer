...
def proof_curation(te_trimmer_proof_curation_dir, output_dir, genome_file, consensus_lib):
    ...
    global te_trimmer_proof_curation_dir_g, output_dir_g, genome_file_g, consensus_lib_g, \
        consensus_folder, others_dir, other_cons_lib_result_folder
    ...
    def define_output_path(local_output_dir=None):
        ...
        return local_output_dir, local_consensus_folder, local_others_dir, local_other_cons_lib_result_folder

    output_dir_g, consensus_folder, others_dir, other_cons_lib_result_folder = define_output_path(output_dir_g)
    ...
    def do_genome_length_calculation(genome_f):
        genome_length_f = os.path.join(bin_py_path, f"{os.path.basename(genome_f)}.length")
        try:
        ...
        def _extension_function(event, chrom_s=chrom_s):
            ...
            if input_fasta_n.lower().endswith(('.fa', '.fasta')):
                ...
                try:
                    ...
                    click.echo(f"An error occurred during genome length file generation: \n {traceback.format_exc()}")
                    messagebox.showerror("Error", f"Genome length file generation failed, please make sure you provide"
                                                  f" FASTA format file. Refer to terminal for more information.",
                                         parent=current_win)
                    return
    ...
    def crop_end_div_gui(input_fasta_n, button, source_dir, output_dir_g, current_win, child_frame, child_canvas,
    ...
    def crop_end_gap_gui(input_fasta_n, button, source_dir, output_dir_g, current_win, child_frame, child_canvas,
                         update_child_canvas=True, file_start=0, file_end=500, save_path=None):
    ...
    def remove_gaps_with_similarity_check_gui(input_fasta_n, button, source_dir, output_dir_g, current_win,
        ...
        def _generate_cons(envet):
            ...
            if input_fasta_n.lower().endswith(('.fa', '.fasta')):
                ...
                messagebox.showerror("Error",
                                     ...
                                     f"(file name end with .fa or .fasta)",
                                     parent=current_win)
    ...
    def define_paths():
        ...
        Label(paths_window, text="TEtrimmer Proof Curation Dir:").grid(row=0, column=0, sticky='w', padx=10, pady=5)
        te_trimmer_entry = Entry(paths_window, width=50)
        te_trimmer_entry.grid(row=0, column=1, padx=10, pady=5, sticky='ew')
        Button(paths_window, text="Browse", command=lambda: browse_directory(te_trimmer_entry)).grid(row=0, column=2,
                                                                                                     padx=5, pady=5)
        Label(paths_window, text="Path to the TEtrimmer_for_proof_curation directory.", fg="gray").grid(row=1, column=1,
                                                                                                        sticky='w',
                                                                                                        padx=10)

        Label(paths_window, text="Output Directory:").grid(row=2, column=0, sticky='w', padx=10, pady=5)
        output_entry = Entry(paths_window, width=50)
        output_entry.grid(row=2, column=1, padx=10, pady=5, sticky='ew')
        Button(paths_window, text="Browse", command=lambda: browse_directory(output_entry)).grid(row=2, column=2,
                                                                                                 padx=5, pady=5)
        Label(paths_window, text="Path where the output files will be saved. Default: Proof Curation Dir",
              fg="gray").grid(row=3, column=1, sticky='w', padx=10)

        Label(paths_window, text="Genome File:").grid(row=4, column=0, sticky='w', padx=10, pady=5)
        genome_entry = Entry(paths_window, width=50)
        genome_entry.grid(row=4, column=1, padx=10, pady=5, sticky='ew')
        Button(paths_window, text="Browse", command=lambda: browse_file(genome_entry)).grid(row=4, column=2, padx=5,
                                                                                            pady=5)
        ...
        Button(paths_window, text="Browse", command=lambda: browse_file(consensus_entry)).grid(row=6, column=2, padx=5,
                                                                                               pady=5)
        Label(paths_window, text="Path to the any other consensus library file.", fg="gray").grid(row=7, column=1, sticky='w',
                                                                                        padx=10)
        ...
        Button(paths_window, text="Save", command=save_paths).grid(row=8, columnspan=3, pady=20)
    ...
    def copy_file(filename, button, target_directory, source_dir, parent_win):
        def _copy_file(event):
            ...
            if target_directory is None or not os.path.isdir(target_directory):
                messagebox.showerror("Error", f"Output directory isn't defined!",
                                     parent=parent_win)
                return
    ...
    def perform_search_in_window(pattern, frame, canvas):
        ...
        if pattern:
            if current_canvas_content == "tetrimmer_out":
                search_results = search_files_with_pattern(pattern, te_trimmer_proof_curation_dir_g, current_canvas_content)
            else:
    ...
    def create_mean_bar():
        ...
        for i, annotation in enumerate(annotation_folders):
            ...
            if annotation_path is None or not os.path.exists(annotation_path):
                ...
                if i == 0:
                    annotationMenu.add_command(label="TEtrimmer proof curation folder not detected! Define by 'Setting' menu",
                                               command=partial(messagebox.showerror,
                                                               "Error", "Please use the correct input directory by 'Setting' menu."
        ...
        settings_menu.add_command(label="Define input and output path", command=define_paths)
