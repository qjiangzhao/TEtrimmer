...
def clean_and_cluster_MSA(input_file, bed_file, output_dir, gap_threshold=0.8, clean_column_threshold=0.08):
    ...
    if pattern_alignment.select_divergent_column():
        ...
        """
        cluster_MSA = MultipleSequenceAlignmentCluster(pattern_alignment, output_dir, min_lines=10, max_cluster=2)

        """
        Test if cluster number is 0, if so skip this sequence
        If cluster is 0, that means no cluster has line numbers greater than 5. In this case, it will be hard
        ...
        if len(cluster_MSA.filtered_cluster_records) == 0:
            #TODO export those fasta names to a file, do extension for 4000 for both side
            return False
        else:
