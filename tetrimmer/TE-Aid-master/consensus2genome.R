...
consensus2genome=function(query=NULL, db=NULL, evalue=10e-8, FL_thresh=0.9, alpha=0.3, full_alpha=1, auto_y=T, bins=NULL, output = NULL, v_x_line_1 = 0, v_x_line_2 = 0){
  ...
  #perform the blast
  blast=read.table(text=system(paste("blastn -max_target_seqs 10000 -query", query, "-db", db , "-evalue", evalue, "-outfmt '6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send sstrand evalue qcovhsp' | sed 's/#/-/g'"), intern = TRUE))
