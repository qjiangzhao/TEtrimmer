...
from TEaid import low_copy_full_blast_and_terminal_check_plus_teaid_plotting
...
def change_permissions_recursive(input_dir, mode):
    try:
        ...
        logging.error(
            "TEtrimmer doesn't have rights to change permissions. Pleas use sudo to run TEtrimmer"
        )
        return False
    return True
        ...
        if search_type == 'blast':
            ...
            if not os.path.isdir(mmseqs_database_dir):
                ...
                if os.path.exists(f'{mmseqs_database_dir}.dbtype'):
                    logging.info(f'MMseqs2 database created successfully: {mmseqs_database_dir}')

                    # Create index for the MMseqs2 database
                    logging.info('Creating index for MMseqs2 database...')
