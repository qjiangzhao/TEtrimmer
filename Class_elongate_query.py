...
class SequenceElongation:
    ...
    def __init__(self, input_file, genome_file, output_dir):
        self.input_file = input_file
        self.genome_file = genome_file
        self.output_dir = output_dir


    def clean_and_cluster_elongate_MSA(self, input_file, output_dir, clean_column_threshold=0.08):
        ...
        if pattern_alignment_object.select_divergent_column():
            ...
            cluster_MSA_object = MultipleSequenceAlignmentCluster(pattern_alignment, output_dir, min_lines=10, max_cluster=1)
    ...
    def elongate_query_seq(self):
        ...
        try:
            ...
            print(f"Error while running blast for sequence: {input_file_name} elongation. Error: {str(e)}")
            return False
        ...
        try:
            ...
            print(f"Error while checking uniqueness for sequence: {input_file_name} elongation. Error: {str(e)}")
            return False
        ...
        try:
            ...
            bed_out_filter_file = bed_out_filter.process_lines(self.output_dir, threshold=100, top_longest_lines_count=100)
            ...
            fasta_out_flank_file = seq_blast.extract_fasta(bed_out_filter_file, self.genome_file, self.output_dir, left_ex=1500, right_ex=1500)
            ...
            # otherwise it will return the subset intact MSA file absolute path
        ...
        try:
            if cluster_MSA_result is False:  # When it is False means all clusters' sequence number is less than 10
                return False
            elif cluster_MSA_result is True:  # When it is True means not necessary to do elongation
                return False
            else:
                # Generate consensus sequence
                output_con = seq_blast.con_generater(str(cluster_MSA_result[0]), self.output_dir, threshold=0.8, ambiguous="N")

                # Change consensus sequence name, otherwise it will be too long for the further analysis
                new_output_con = os.path.join(self.output_dir, f"{input_file_name}_elongate.fa")
                os.rename(output_con, new_output_con)

            return new_output_con

        except Exception as e:
            print(f"Error during consensus generation for sequence: {input_file_name} elongation Error: {str(e)}")
            traceback.print_exc()

"""
input_file = "/work/ur376715/TE_analysis/Software_develpment_for_TE_curation/version_18/output_dir/Single_fasta_files/LTRRT_15.fasta"
genome_file = "/work/ur376715/TE_analysis/TE_manual_curation/Genome_BLAST_database_for_TE_manual_curation/bgh_dh14_v4.fa"
output_dir = "/work/ur376715/TE_analysis/Software_develpment_for_TE_curation/version_19"

test = SequenceElongation(input_file, genome_file, output_dir)
test_result = test.elongate_query_seq()
print(test_result)
"""