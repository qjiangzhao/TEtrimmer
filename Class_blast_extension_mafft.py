...
class SequenceManipulator:
    ...
    def calculate_genome_length(self, genome_file):
        ...
        :return: dict, dictionary of sequence names and lengths
        """
        genome_lengths = {}
        ...
        with open(output_file, "w") as f:
            for seq_name, seq_length in genome_lengths.items():
    ...
    def check_database(self, genome_file):
        ...
        """
        database_file = genome_file + ".nin"
        if not os.path.isfile(database_file):
            print(f"\n\nBlast database doesn't exist. Running makeblastdb, this can take some time\n\n")
            # the default output path for makeblastdb is the same directory as the input FASTA file
            makeblastdb_cmd = f"makeblastdb -in {genome_file} -dbtype nucl"
    ...
    def blast(self, input_file, genome_file, output_dir, min_length=200):
        """
        ...
        :param min_length: num default 200, minimum alignment length
    ...
    def check_bed_uniqueness(self, output_dir, bed_file):
        ...
        for entry in entries:
            ...
            if key not in unique_keys:
                ...
                print(f"Duplicate entry found and removed: {entry}")
    ...
    def align_sequences(self, input_file, output_dir):
        ...
        # Mafft will automatically converte all to lowercase in the output
        ...
        subprocess.run(mafft_cmd, shell=True, check=True, stdout=subprocess.DEVNULL)
        return fasta_out_flank_mafft_file

    def con_generater(self, input_file, output_dir, threshold=0.8, ambiguous="X"):
        # Read input file
        ...
        output_file = os.path.join(output_dir, f"{os.path.basename(input_file)}_TEaid.fa")
    ...
    def select_start_end_and_join(self, input_file, output_dir, start, end, window_size=50):
        ...
        :param window_size: int default 50, columns size will be selected for each sides
        :return: Selected MSA object (not file path)
        """
        alignment = AlignIO.read(input_file, "fasta")
        sequence_length = end - start

        if sequence_length >= 200:  # Multiple sequence length have to be longer than 200bp
            ...
            raise ValueError(f"The alignment can't be shorter than 200bp")
    ...
    def select_window_columns(self, input_file, output_dir, start_point, direction, window_size=50):
        ...
        :return: Selecte MSA object (not file path)
        """
...
"""
input_file = "G:\\TE_manual_curation\\Software_develop\\Test_for_new_gap_and_crop_by_gap\\LTR_example.fasta"
output_dir = "G:\\TE_manual_curation\\Software_develop\\Test_for_new_gap_and_crop_by_gap"

test = SequenceManipulator()
test1 = test.remove_gaps_with_similarity_check(input_file, output_dir, gap_threshold=0.7, simi_check_gap_thre=0.4,
...
test = SequenceManipulator()
test1 = test.remove_gaps(input_file, output_dir, threshold=0.4)

"""
"""
file = "G:\\TE_manual_curation\\Software_develop\\Test_for_new_gap_and_crop_by_gap\\LTR_example.fasta"  # replace with your filename
output_file = "G:\\TE_manual_curation\\Software_develop\\Test_for_new_gap_and_crop_by_gap"  # replace with your desired output filename
start, end = 343, 6292  # replace with your start and end points
test = SequenceManipulator()
output, new_start, new_end = test.process_alignment(file, output_file, start, end)
print(f"New start point: {new_start}")
print(f"New end point: {new_end}")
