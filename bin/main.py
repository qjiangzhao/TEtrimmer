    ...
    try:
        ...
        if cluster_MSA_result is False:
            ...
            find_boundary_and_crop(bed_out_filter_file, genome_file, MSA_dir,
                                   pfam_dir, seq_name, cons_threshold=cons_thr, ext_threshold=ext_thr,
            ...
            if all_inner_skipped:
                with open(skipped_file, "a") as f:
                    f.write(seq_name + f"\tcluster_blast_smaller_than_{min_seq_num}\n")
                    click.echo(
                        f"{seq_name} is skipped due to sequence number in second round each cluster is "
...
@click.option('--cons_thr', default=0.8, type=float,
              help='Threshold used for the final consensus sequence generation')
...
@click.option('--min_blast_len', default=200, type=int,
              help='The minimum hit sequence length for blast')
@click.option('--max_cluster_num', default=2, type=int,
              help='The maximum cluster number for each multiple sequence alignment. Each multiple sequence alignment '
                   ...
                   'the top --max_cluster_num of clusters for the further analysis')
@click.option('--min_el', default=2000, type=int,
              help='If the query sequence length is smaller than the given number, TE Trimmer will elongate the query '
                   'sequence. This can help identify the intact TE')
...
@click.option('--ext_thr', default=0.7, type=float,
              help="threshold used for define the extension extent. The smaller number means it become easier "
                   "to have a final longer extension for each side of the sequence.")
...
@click.option('--gap_thr', default=0.4, type=float,
              help='If columns have a larger gap proportion than --gap_thr and the most common nucleotide proportion '
                   'in this column is less than --gap_nul_thr, this column will be removed')
...
@click.option('--start_patterns', default='TG', type=str,
              help='LTR elements will always start with fixed pattern. TE Trimmer will check if it starts with those '
                   ...
                   'Like: TG,TA,TC (No space between them). The order of the given patterns is matter')
@click.option('--end_patterns', default='CA', type=str,
              help='LTR elements will always end with fixed pattern. TE Trimmer will check if it end with those '
                   ...
                   'CA,TA,GA (No space between them). The order of the given patterns is matter')
