...
def analyze_sequence(seq_name, Single_files_dir, genome_file, MSA_dir, min_blast_len, min_seq_num, max_msa_lines,
                     ...
                     start_patterns, end_patterns, output_dir, pfam_dir, mini_orf, elongation_dir):
    ...
    try:
        if cluster_MSA_result is False:
            ...
            find_boundary_and_crop(bed_out_filter_file, genome_file, MSA_dir,
                                   pfam_dir=pfam_dir, cons_threshold=cons_thr,
...
@click.option('--continue_analysis', default=False, is_flag=True,
              help='if --continue_analysis is provided on the command line, TE Trimmer will continue the analysis'
                   'based on the existed data. Otherwise, it will overlap the existed files')
...
@click.option('--min_el', default=2000, type=int,
              help='If query sequence length is smaller than the given number, 1500bp will be added to the left and '
                   'right side of this sequence. Meanwhile, redundant elongation will be cleaned')
@click.option('--min_el_dna', default=500, type=int,
              help='Same like --min_el option. But set different given number for DNA element. Default: 500')
...
@click.option('--ex_step', default=1000, type=int,
              help='Number of nucleotide will be added to left or right side of multiple sequence alignment. TE_Trimmer'
                   'will iteratively add --ex_step number of nucleotide until find the boundary')
@click.option('--max_extension', default=7000, type=int,
              help='The maximum extension number for right and left side. For example, if --ex_step is 1000, it can '
                   'only add seven times to the MSA left or right side')
@click.option('--gap_thr', default=0.4, type=float,
              help='If columns have larger gap proportion than --gap_thr and the most common nucleotide proportion in '
...
@click.option('--start_patterns', default='TG', type=str,
              help='LTR elements will always start with fixed pattern. TE Trimmer will check if it start with those'
                   'patterns. If not, it will seek around the start point, if the pattern is found, the start point '
                   'will be converted to there. Note: if you want give multiple start patterns, separate them by comma.'
...
@click.option('--end_patterns', default='CA', type=str,
              help='LTR elements will always end with fixed pattern. TE Trimmer will check if it end with those'
                   ...
                   'be converted to there. Note: if you want give multiple end patterns, separate them by comma. Like:'
...
def main(input_file, genome_file, output_dir, continue_analysis, pfam_dir, min_blast_len, num_threads, max_msa_lines,
    ...
    input_fasta_dir = os.path.dirname(os.path.abspath(input_file))  # get genome path
    ...
    if continue_analysis:
        ...
        sequence_names = [f for f in os.listdir(Single_files_dir) if
                          f.endswith(".fasta") and f not in completed_sequences]
    else:
    ...
    analyze_sequence_params = [
        (seq_name, Single_files_dir, genome_file, MSA_dir, min_blast_len, min_seq_num, max_msa_lines,
         top_mas_lines, max_cluster_num, min_el, min_el_dna, min_el_sine, cons_thr, ex_step,
         max_extension, gap_thr, gap_nul_thr, crop_end_thr, crop_end_win, crop_end_gap_thr, crop_end_gap_win,
         start_patterns, end_patterns, output_dir, pfam_dir, mini_orf, elongation_dir) for seq_name in sequence_names]
