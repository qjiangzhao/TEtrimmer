...
def analyze_sequence(seq_name, Single_files_dir, genome_file, MSA_dir, min_blast_len, min_seq_num, max_msa_lines,
    ...
    try:
        ...
        if seq_blast.blast_hits_count == 0:
            with open(skipped_file, "a") as f:
                f.write(seq_name + "\tblast_equal_0\n")
        ...
        # check if blast hit number is smaller than "min_seq_num", not include "min_seq_num"
            ...
            with open(skipped_file, "a") as f:
                f.write(seq_name + f"\tblast_smaller_than_{min_seq_num}\n")
    ...
    try:
        ...
        if cluster_MSA_result is False:
            with open(skipped_file, "a") as f:
                f.write(seq_name + f"\tcluster_blast_smaller_than_{min_seq_num}\n")
            click.echo(f"\n{seq_name} is skipped due to sequence number in each cluster is smaller than {min_seq_num}\n")
            return
        elif cluster_MSA_result is True:
            find_boundary_and_crop(bed_out_filter_file, genome_file, MSA_dir, pfam_dir, seq_name, hmm_dir,
                                   skipped_file, cons_threshold=cons_thr, ext_threshold=ext_thr,
            ...
            for i in range(len(cluster_pattern_alignment_list)):
                ...
                if inner_cluster_MSA_result is False:
                    ...
                    find_boundary_and_crop(cluster_bed_files_list[i], genome_file, MSA_dir, pfam_dir, seq_name,
                                           hmm_dir, skipped_file, cons_threshold=cons_thr, ext_threshold=ext_thr,
                                           ex_step_size=ex_step, max_extension=max_extension,
                                           gap_threshold=gap_thr, gap_nul_thr=gap_nul_thr,
                                           crop_end_thr=crop_end_thr, crop_end_win=crop_end_win,
                                           crop_end_gap_thr=crop_end_gap_thr, crop_end_gap_win=crop_end_gap_win,
                    ...
                    for j in range(len(inner_cluster_bed_files_list)):
                        find_boundary_and_crop(inner_cluster_bed_files_list[j], genome_file, MSA_dir,
                                               pfam_dir, seq_name, hmm_dir, skipped_file,
...
@click.option('--cons_thr', default=0.8, type=float,
...
@click.option('--min_blast_len', default=150, type=int,
...
@click.option('--min_el_sine', default=100, type=int,
              help='Same like --min_el option. But set different given number for SINE element. Default: 100')
@click.option('--ext_thr', default=0.7, type=float,
...
@click.option('--crop_end_gap_win', default=150, type=int,
              help='Define window size used to crop end by gap, coupled with --crop_end_gap_thr option. Default: 150')
@click.option('--crop_end_gap_thr', default=0.1, type=float,
...
def main(input_file, genome_file, output_dir, continue_analysis, pfam_dir, min_blast_len, num_threads, max_msa_lines,
    ...
    def check_progress_file(progress_file_path):
        ...
        try:
            ...
            error_message = f"The file {progress_file_path} isn't found. Please change the -o option to the right " \
                            f"output directory."
            click.echo(error_message)
            raise FileNotFoundError(error_message)  # Raise the error with a customized message
