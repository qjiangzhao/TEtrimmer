...
class SequenceElongation:
    ...
    def __init__(self, input_file, genome_file, output_dir, skipped_file, min_seq_num=10, crop_end_gap_win=150,
                 crop_end_gap_thr=0.05, ext_n=1500):
    ...
    def elongate_query_seq(self):
        ...
        try:
            ...
            crop_end_by_gap_object = CropEndByGap(fasta_out_flank_file_MSA_gap_block_sim,
                                                  gap_threshold=self.crop_end_gap_thr,
                                                  window_size=self.crop_end_gap_win)
        ...
        try:
            ...
            # Calculate final consensus sequence length, if length is shorter than 100, skip this sequence
            output_con_len = seq_blast.calcuate_seq_length(output_con)

            if output_con_len < 150:
                with open(self.skipped_file, "a") as f:
                    f.write(input_file_name + f"\telement is too short for TE Trimmer\n")
