...
def find_boundary_and_crop(bed_file, genome_file, output_dir, pfam_dir, seq_obj, hmm, classify_all, classify_unknown,
    ...
    try:
        ...
        # because LINEs have high divergence at the 5' end
        ...
        while msa_loop_n <= 2:
            ...
            try:
                ...
                # final_MSA returns 'False' if the start crop point is greater than the end crop point, which means the
        ...
        #####################################################################################################
            ...
            if start_patterns is not None or end_patterns is not None and not found_match_crop:
                if checkpattern.is_LTR(cropped_alignment_output_file_g):  # Check if file name contains "LTR"
        ...
        #####################################################################################################
        # Code block: Concatenate beginning and end windows of the MSA and plot
        #####################################################################################################
            ...
            if orf_domain_plot_object.run_getorf():
                ...
                if if_pfam_domain:
                    ...
                    if determine_sequence_direction(pfam_result_file):
                        ...
                        # get_alignment_length() is a built-in function
        ...
        #####################################################################################################
            ...
            with open(error_files, "a") as f:
                ...
                f.write('\n' + tb_content + '\n\n')
                prcyan(f"\nMSA plot failed for {seq_name} with error: \n{e}")
                prgre("\nMSA plots are only used to evaluate TE Trimmer and will not affect the final TE consensus library."
                      "For traceback text, please refer to 'error_file.txt' in the 'Multiple_sequence_alignment' folder\n")
                pass
        ...
        try:
            ...
            # The TE-Aid package is stored in the same directory as this function file.
        ...
        try:
            # Because dotmatcher cannot change output resolution, scale it up to make plot clearer in the merged PDF
            ...
            with open(error_files, "a") as f:
                # Get the traceback content as a string
                tb_content = traceback.format_exc()
                f.write(f"\nPostscript to PDF conversion failed for {seq_name} with error:\n{e}")
                f.write('\n' + tb_content + '\n\n')
        ...
        try:
            ...
            # "proof" refers to the manual inspection of consensus elements via MSA plotting
            proof_annotation_dir = os.path.join(parent_output_dir, "TE_Trimmer_for_proof_annotation")

            # Construct the path for the classification_dir folder
            ...
            # Storing consensus sequence into consi_obj is not necessary
        ...
        try:
            if classify_all or (
                ...
                with open(classification_seq_file, "w") as f:
                    # RepeatClassifier input cannot be a single sequence, add >Add to enable RepeatClassifier run
        ...
        #####################################################################################################
        # Code block: Move file for manual inspection and HMM search
        #####################################################################################################
        ...
        with open(error_files, "a") as f:
            # Get the traceback content as a string
            tb_content = traceback.format_exc()
            f.write(f"\ncrop_end_and_remove_gap failed for {seq_name} with error:\n{e}")
            f.write('\n' + tb_content + '\n\n')
            prcyan(f"\ncrop_end_and_remove_gap failed for {seq_name} with error:\n{e}")
            prcyan('\n' + tb_content + '\n')
            raise Exception
