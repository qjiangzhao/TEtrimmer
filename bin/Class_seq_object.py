...
class Seq_object:
    ...
    def create_consi_obj(self, consi_name):
        consi_obj = Consensus_object(self, consi_name)  # Pass the parent seq_object as an argument
...
class Consensus_object:
    ...
    def get_TE_type_for_file(self):
        # for writing the final consensus file, the TE_type can be either the input TE type or the new classified one if it is newly classififed
        if "N/A" in self.new_TE_type:
