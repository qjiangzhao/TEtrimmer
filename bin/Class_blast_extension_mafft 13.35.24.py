...
class SequenceManipulator:
    ...
    def check_database(self, genome_file):
        ...
        :param output_dir: str, path to create blast database files
        """
        # Get genome file directory
        genome_file_dir = os.path.dirname(genome_file)

        # Define folder under genome file directory to store blast database
        # This folder is created in main.py
        blast_database_dir = os.path.join(genome_file_dir, "TE_Trimmer_blast_db")

        # Get genome file name
        genome_file_n = os.path.basename(genome_file)

        # Define blast database name, database has to be in the different folder with genome file and with different name
        blast_database_name = os.path.join(blast_database_dir, f"{genome_file_n}_db")

        blast_database_file = blast_database_name + ".nin"
        if not os.path.isfile(blast_database_file):
            print(f"\nBlast database doesn't exist. Running makeblastdb, this can take some time\n")

            makeblastdb_cmd = f"makeblastdb -in {genome_file} -dbtype nucl -out {blast_database_name} "
    ...
    def blast(self, input_file, genome_file, output_dir, min_length=200):
        ...
        blast_cmd = f"blastn -query {input_file} -db {blast_database_name} " \
